...
resource "null_resource" "frontend" {
    ...
    provisioner "file" {    #to copy files from source to destionation server
...
resource "aws_ec2_instance_state" "frontend" {
  ...
  state       = "stopped"
  depends_on = [ null_resource.frontend ] #stop the server only when null resource provisioning completed
}
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition { 
    host_header {     #this is the host path rule
